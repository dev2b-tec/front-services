...
function ModalAdicionar({
  empresaId, profissionais, onClose, onSaved,
}: {
  empresaId: string
...
function ModalEditar({
  ...
  const [pago,      setPago]      = useState(fmtBRL(row.valorPago))
  const [status,    setStatus]    = useState(row.status)
  const [metodo,    setMetodo]    = useState(row.metodoPagamento ?? '')
  const [dataPag,   setDataPag]   = useState(row.dataPagamento ?? '')
  const [saving,    setSaving]    = useState(false)

  const METODOS = ['Dinheiro', 'Pix', 'Cartão de Crédito', 'Cartão de Débito', 'Transferência', 'Boleto', 'Outros']
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[var(--d2b-bg-elevated)] border border-[var(--d2b-border-strong)] rounded-2xl shadow-2xl w-full max-w-xl mx-4">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {/* Valor + Valor Pago + Efetuar Pagamento */}
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              ...
              <input
                ...
                inputMode="numeric"
                value={pago}
                onChange={(e) => setPago(maskBRL(e.target.value) || e.target.value)}
                className={INP}
              />
            </div>
            <button
            ...
            >
              <Check size={13} /> Efetuar Pagamento
            </button>
          </div>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export function TabContasPagar({ empresaId }: { empresaId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/movimentos-financeiros/empresa/${empresaId}?tipo=A_PAGAR`
      )
      if (res.ok) {
        const data = await res.json()
        setRows(data.map((m: {
          id: string; titulo: string; usuarioId?: string; usuarioNome?: string
          tipo: string; grupoId: string; numeroParcela: number; totalParcelas: number
          valorParcela: number; valorPago: number; dataVencimento: string
          dataPagamento?: string; status: string; metodoPagamento?: string
        }) => ({
          id:             m.id,
          titulo:         m.titulo,
