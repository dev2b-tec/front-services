  ...
  return (
    <div className="flex flex-col items-center justify-center h-full min-h-[60vh] p-8 text-center">
      ...
      <div className="relative mb-6">
        <div className="w-20 h-20 rounded-2xl bg-[#150830] border border-[rgba(124,77,255,0.25)] flex items-center justify-center">
          <Construction size={36} className="text-[#7C4DFF]" strokeWidth={1.5} />
        </div>
        <div className="absolute -bottom-2 -right-2 w-8 h-8 rounded-lg bg-[#120328] border border-[rgba(124,77,255,0.18)] flex items-center justify-center">
          <Puzzle size={14} className="text-[#C084FC]" />
        </div>
      </div>

      {/* Título */}
      <h2 className="text-xl font-bold text-[#F5F0FF] text-balance mb-2">
        {modulo}
      </h2>
      <p className="text-sm text-[#A78BCC] max-w-xs leading-relaxed mb-6">
        Este módulo ainda está em construção e será disponibilizado em breve.
      </p>

      {/* Instrução para o dev */}
      {caminho && (
        <div className="w-full max-w-sm rounded-xl border border-[rgba(124,77,255,0.18)] bg-[#120328] p-4 text-left">
          <p className="text-[10px] font-mono text-[#7C4DFF] uppercase tracking-widest mb-2">
            Para implementar este módulo:
          </p>
          <p className="text-xs font-mono text-[#A78BCC] leading-relaxed">
            <span className="text-[#C084FC]">1.</span> Crie o arquivo:
            <br />
            <span className="ml-3 text-[#F5F0FF] bg-[#0D0520] rounded px-2 py-0.5 inline-block mt-1">
              {caminho}
            </span>
            <br />
